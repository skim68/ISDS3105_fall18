...
ggplot(twitter_users,aes(x = Year, y = millions)) + 
...
```{r}
ggplot(data=beerDt,aes(x=Year,y=GallonsCapita))+
  geom_line(color='blue')+
  geom_point()
...
```{r}
myBirthYear <- rep(FALSE,30)
year <- seq(1981,2010)
myBirthYear <- ifelse(year==1990,TRUE,FALSE)
myData <- data.frame(cbind(year,myBirthYear))
ggplot(data=myData)+
  geom_point(aes(x=year,y=0,fill=myBirthYear))+
  ggtitle('Hi')
